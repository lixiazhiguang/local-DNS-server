...
int debug_level = 0;
char DNS_ADDRESS[16] = "10.3.9.4";

IDTransform IDTransTable[AMOUNT];  // ID转换表
int IDcount = 0;                   //转换表中的条目个数

SOCKET local_sock, remote_sock;  //创建本地套接字，和外部套接字
int len_client;
struct sockaddr_in local_name, outside_name;  // AF_INET地址
...
uint16_t RegisterNewID(uint16_t oID, SOCKADDR_IN temp, bool ifdone) {
  int i = 0;
  for (i = 0; i != AMOUNT; ++i) {
    //找到已过期或已完成请求的ID位置覆盖
  ...
  if (i == AMOUNT)  //没找到可写的地方
    return 0;
  return (uint16_t)i + 1;  //以表中下标作为新的ID
}
...
void readurl(char* buf, char* dest) {
  ...
  while (i < len) {
    if (buf[i] > 0 && buf[i] <= 63)  //如果是个计数
    {
      ...
      for (j = buf[i], i++; j > 0; j--, i++, k++) dest[k] = buf[i];
    }
...
void receive_from_out() {
  ...
  //接受外部DNS报文消息
  length = recvfrom(remote_sock, buf, sizeof(buf), 0,
                    (struct sockaddr*)&client2, &len_client);

  if (length > -1) {
    if (debug_level >= 2) standard_print(buf, length);

    //以进程ID来作为DNS报文的一个随机标示符
    uint16_t* pID = (uint16_t*)malloc(sizeof(uint16_t));
    memcpy(pID, buf, sizeof(uint16_t));
    int id_index = (*pID) - 1;
    free(pID);

    uint16_t oID = IDTransTable[id_index].oldID;  //转换为客户端方向的ID
    memcpy(buf, &oID, sizeof(uint16_t));
    ...
    //从表中找到此条DNS请求的客户端发送者
    client = IDTransTable[id_index].client;

    int nquery = ntohs(*((uint16_t*)(buf + 4))),
        nresponse = ntohs(*((uint16_t*)(buf + 6)));  //问题个数；回答个数
    ...
    for (j = 0; j < nresponse; ++j) {
      ...
      uint16_t resp_type = ntohs(*(uint16_t*)p);  //回复类型
      p += 2;
      uint16_t resp_class = ntohs(*(uint16_t*)p);  //回复类
      p += 2;
      uint16_t high = ntohs(*(uint16_t*)p);  //生存时间高位
      p += 2;
      uint16_t low = ntohs(*(uint16_t*)p);  //生存时间低位
      p += 2;
      int ttl = (((int)high) << 16) | low;  //高低位组合成生存时间
      int datalen = ntohs(*(uint16_t*)p);   //后面数据长度
      p += 2;
      if (debug_level >= 2)
...
void receive_from_local() {
  ...
  if (length > 0) {
    ...
    // 获取请求报文中的域名表示
    memcpy(ori_url, &(buf[sizeof(DNS_HDR)]), length);
        ...
        ip.addr[0] == 'e')  //在缓存中未找到对应的IP或者该域名对应的IP已经过期
    {
      ...
      uint16_t* pID = (uint16_t*)malloc(sizeof(uint16_t));
      memcpy(pID, buf, sizeof(uint16_t));  //记录ID
      //储存ID和该发送方的地址client
      uint16_t nID = RegisterNewID(*pID, client, FALSE);
      if (nID == 0) {
        if (debug_level >= 1) puts("Buffer full.");
      } else {
        if (debug_level >= 1) printf("send outside %s\n", url);
        memcpy(buf, &nID, sizeof(uint16_t));
        //将该请求发送给外部服务器
        length = sendto(remote_sock, buf, length, 0,
                        (struct sockaddr*)&outside_name, sizeof(outside_name));
      }
      free(pID);
      ...
      uint16_t a = htons(0x8180);
      memcpy(&sendbuf[2], &a, sizeof(uint16_t));  //修改标志域
      ...
      memcpy(&sendbuf[6], &a, sizeof(uint16_t));

      int curLen = 0;
      char answer[16];
      uint16_t Name = htons(0xc00c);  //域名指针（偏移量）
      memcpy(answer, &Name, sizeof(uint16_t));
      curLen += sizeof(uint16_t);

      uint16_t TypeA = htons(0x0001);  //类型
      memcpy(answer + curLen, &TypeA, sizeof(uint16_t));
      curLen += sizeof(uint16_t);

      uint16_t ClassA = htons(0x0001);  //查询类
      memcpy(answer + curLen, &ClassA, sizeof(uint16_t));
      curLen += sizeof(uint16_t);
      ...
      uint16_t IPLen = htons(0x0004);  //资源数据长度
      memcpy(answer + curLen, &IPLen, sizeof(uint16_t));
      curLen += sizeof(uint16_t);
...
int main(int argc, char* argv[]) {
  ...
  //储存调用AfxSocketInit全局函数返回的Windows Sockets初始化信息
  WSADATA wsaData;
  WSAStartup(MAKEWORD(2, 2), &wsaData);           //初始化Winsock服务
  local_sock = socket(AF_INET, SOCK_DGRAM, 0);    //创建本地套接字
  remote_sock = socket(AF_INET, SOCK_DGRAM, 0);  //创建外部套接字

  int unblock = 1;
  //将外部套街口设置为非阻塞
  ioctlsocket(remote_sock, FIONBIO, (u_long FAR*)&unblock);
  //将本地套街口设置为非阻塞
  ioctlsocket(local_sock, FIONBIO, (u_long FAR*)&unblock);
  if (local_sock < 0) {
